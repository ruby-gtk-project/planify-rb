...
### GLib::DateTime — keyword options, all six required

The C API's positional constructor does not exist in the Ruby bindings, and a
year/month/day-only hash is rejected. Pass all six:

```ruby
# WRONG - "wrong number of arguments (given 6, expected 1)"
GLib::DateTime.new(2030, 5, 1, 0, 0, 0)

# WRONG - ":unix or (:year, :month, :day, :hour, :minute and :second) must be specified"
GLib::DateTime.new(year: 2030, month: 5, day: 1)

# RIGHT
GLib::DateTime.new(year: 2030, month: 5, day: 1, hour: 0, minute: 0, second: 0)
```

It also has no `to_time`. Read a `Gtk::Calendar` selection back field by field:

```ruby
calendar.date.then { |d| Time.new(d.year, d.month, d.day_of_month) }
```

### Gtk::IconTheme — `get_for_display`, not `for_display`

```ruby
# WRONG - undefined method 'for_display'
Gtk::IconTheme.for_display(Gdk::Display.default)

# RIGHT
Gtk::IconTheme.get_for_display(Gdk::Display.default)
```

### Gdk::RGBA.parse raises instead of returning false

The C function returns a boolean; the Ruby binding raises `ArgumentError` on
an unparseable string, so a "parse it if you can" fallback needs a rescue,
not an `if`:
...
### Gtk::EmojiChooser is a popover — give it a MenuButton

`set_parent` on a plain `Gtk::Button` works but leaks:
"Finalizing GtkButton, but it still has children left: GtkEmojiChooser".
Use `Gtk::MenuButton#popover=` instead.

### Memoized windows: actions registered inside the tap see a nil memo

`@window ||= Adwaita::ApplicationWindow.new(app).tap { |win| ... }` has not
assigned `@window` yet while the tap body runs, so a helper called from inside
it that reads `window` gets nil. Pass `win` in:

```ruby
# WRONG - "undefined method 'add_action' for nil"
def register_actions = window.add_action(...)

# RIGHT
def register_actions(win) = win.add_action(...)
```
