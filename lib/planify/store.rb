...
module Planify
  ...
  class Store
    ...
    attr_reader :database, :projects, :sections, :items, :labels, :reminders, :attachments
    ...
    def delete_project(project)
      subprojects_of(project.id).each { |child| delete_project(child) }
    ...
    def delete_section(section)
      items_by_section(section.id).each { |item| delete_item(item) }
    ...
    def delete_item(item)
      subitems_of(item.id).each { |subitem| delete_item(subitem) }
      @items.delete(item)
